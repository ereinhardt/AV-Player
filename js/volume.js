  ...
  tracks.forEach((track) => {
    ...
    if (isVideoTrack) {
      ...
      ["left", "right"].forEach((side) => {
        const volumeSlider = track.querySelector(
          `#volume-slider-${index}-${side}`
        );
        const volumeDbDisplay = track.querySelector(
          `#volume-db-${index}-${side}`
        );
        const muteCheckbox = track.querySelector(
          `#mute-checkbox-${index}-${side}`
        );
