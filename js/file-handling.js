...
function cleanupAudioElement(audioElement) {
  if (audioElement && audioElement._blobUrl) {
    URL.revokeObjectURL(audioElement._blobUrl);
    audioElement._blobUrl = null;
  }
}

// Disconnect Web Audio API nodes to prevent memory leaks
function disconnectAudioNodes(audioSource) {
  if (!audioSource) return;

  try {
    if (audioSource.source) audioSource.source.disconnect();
    if (audioSource.merger) audioSource.merger.disconnect();
    if (audioSource.gainNode) audioSource.gainNode.disconnect();
    if (audioSource.splitter) audioSource.splitter.disconnect();
    if (audioSource.leftGainNode) audioSource.leftGainNode.disconnect();
    if (audioSource.rightGainNode) audioSource.rightGainNode.disconnect();
  } catch (error) {
    console.warn("Error disconnecting audio nodes:", error);
  }
}

// Clean up all audio elements in the array
...
// Clean up all audio sources and their Web Audio nodes
function cleanupAllAudioSources(audioSources) {
  audioSources.forEach((audioSource) => {
    if (audioSource) {
      disconnectAudioNodes(audioSource);
    }
  });
}

// Validate that uploaded file matches expected track type
function validateFileType(file, isVideoTrack, index) {
  ...
  if (!file.type.startsWith(expectedType)) {
    alert(
      `Please select a ${fileType}. Track ${index} detected as ${
        isVideoTrack ? "video" : "audio"
      } track but got ${file.type}`
    );
    return false;
  }
  return true;
...
// Set up audio context and configure audio device for a track
async function setupAudioContext(
  ...
  audioContextContainer.masterGains[index] = createConfiguredMasterGain(
    context,
...
// Set up timeline progress and time display event handlers
...
// Set up file input handlers for audio and video tracks
    ...
    fileInput.addEventListener("change", async (event) => {
      ...
      if (audioElements[index]) {
        if (
          ...
          audioElements[index].src.startsWith("blob:")
        ) {
      ...
      if (audioDeviceSelect?.value) {
        await setAudioDeviceUnified(
          audio,
          audioContextContainer.contexts[index],
          audioDeviceSelect.value
        );
      }

      setupTimelineHandlers(audio, timelineProgress, timeDisplay);
      setupWebAudioConnections(
        audio,
        index,
        audioContextContainer,
        audioSources,
...
// Set up Web Audio connections for stereo and mono tracks
function setupWebAudioConnections(
  audio,
  index,
  audioContextContainer,
  audioSources,
  ...
  const merger = context.createChannelMerger(
    Math.max(context.destination.maxChannelCount, 18)
  );
...
// Cleanup blob URLs when page is unloaded to prevent memory leaks
window.addEventListener("beforeunload", () => {
  if (window.audioElements) {
    cleanupAllAudioElements(window.audioElements);
  }
  if (window.audioSources) {
    cleanupAllAudioSources(window.audioSources);
  }
});
