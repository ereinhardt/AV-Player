    ...
    tracks.forEach((track, arrayIndex) => {
        ...
        fileInput.addEventListener('change', async (event) => {
            ...
            if (isVideoTrack && !file.type.startsWith('video/')) {
                alert(`Please select a video file (MP4). Track ${index} detected as video track but got ${file.type}`);
                return;
            } else if (!isVideoTrack && !file.type.startsWith('audio/')) {
                alert(`Please select an audio file (MP3, WAV). Track ${index} detected as audio track but got ${file.type}`);
                return;
            }
            ...
            if (!audioContextContainer.contexts[index]) {
                ...
                audioContextContainer.masterGains[index] = audioContextContainer.contexts[index].createGain();
                audioContextContainer.masterGains[index].channelCount = destination.channelCount;
                audioContextContainer.masterGains[index].channelCountMode = 'explicit';
                audioContextContainer.masterGains[index].channelInterpretation = 'discrete';
                ...
                if (audioDeviceSelect && audioDeviceSelect.value) {
                    ...
                    try {
                        ...
                        try {
                            if (typeof audioContextContainer.contexts[index].setSinkId === 'function') {
                                await audioContextContainer.contexts[index].setSinkId(audioDeviceSelect.value);
                            }
                        } catch (fallbackError) {
                ...
                if (audioContextContainer.masterVolume !== undefined) {
                    audioContextContainer.masterGains[index].gain.value = audioContextContainer.masterVolume;
                }
            }
            ...
            if (isVideoTrack) {
                ...
                const splitter = audioContextContainer.contexts[index].createChannelSplitter(2);
                const leftGainNode = audioContextContainer.contexts[index].createGain();
                const rightGainNode = audioContextContainer.contexts[index].createGain();
